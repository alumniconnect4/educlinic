...
export default function ManageAdmins() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true)
    try {
      await axios.post("http://localhost:4000/api/admin-portal/admins", formData, { withCredentials: true })
      toast.success("Admin created successfully!")
      setFormData({ name: "", email: "", password: "", role: "ADMIN", schoolCategory: "", avatarUrl: "" })
      fetchAdmins()
    } catch (err: unknown) {
  ...
  const handleUpdateSubmit = async (e: React.FormEvent) => {
    ...
    setIsUpdating(true)
    try {
      await axios.put(`http://localhost:4000/api/admin-portal/admins/${editingAdmin.id}`, editFormData, { withCredentials: true })
  ...
  const executeDelete = async (adminId: number) => {
    try {
      await axios.delete(`http://localhost:4000/api/admin-portal/admins/${adminId}`, { withCredentials: true })
    ...
    <div className="w-full h-[calc(100vh-112px)] min-h-[580px] grid grid-cols-1 xl:grid-cols-[360px_1fr] gap-6 relative">
      ...
      <div className="bg-white border border-gray-200 shadow-sm rounded-sm flex flex-col h-full overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 shrink-0">
          ...
          <p className="text-xs text-gray-500 mt-0.5 font-normal">Register a new administrator account</p>
        </div>
      ...
      <div className="bg-white border border-gray-200 shadow-sm rounded-sm flex flex-col h-full overflow-hidden">
          ...
          <table className="w-full border-collapse">
            ...
            <tbody className="text-sm text-gray-600">
              {admins.length === 0 ? (
                ...
                admins.map((admin) => (
                  <tr key={admin.id} className="border-b border-gray-100 hover:bg-blue-50/40 transition-colors">
                    <td className="py-3.5 px-6 align-middle">
                      ...
                      >
                        ...
                        />
                        <div>
                          <span className="font-semibold text-[#344767] block leading-tight group-hover:text-blue-600 group-hover:underline transition-colors">{admin.name}</span>
                          <span className="text-xs text-gray-400 font-normal">ID: {admin.id}</span>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-3.5 px-6 align-middle text-center">
                      <div className="flex justify-center">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-sm text-xs font-bold border ${
                          ...
                          {admin.role === 'SUPER_ADMIN' && <ShieldCheck className="w-3 h-3 mr-1" />}
        ...
        <div className="px-6 py-3.5 bg-gray-50 border-t border-gray-200 shrink-0 flex items-center justify-between">
          <p className="text-xs text-gray-500 font-medium">
            Showing <span className="font-bold text-gray-700">{total === 0 ? 0 : (currentPage - 1) * itemsPerPage + 1}</span> to <span className="font-bold text-gray-700">{Math.min(currentPage * itemsPerPage, total)}</span> of <span className="font-bold text-gray-700">{total}</span> admins
          </p>
          <div className="flex items-center gap-2">
            <button 
          ...
          <div className="bg-white rounded-lg border border-gray-200 shadow-2xl w-full max-w-xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between bg-slate-900 text-white">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-sm font-bold uppercase tracking-wider">Admin Profile Details</h3>
                  <p className="text-[11px] text-slate-400 font-normal">ID #{viewingAdmin.id} • Administrator Account</p>
                </div>
              </div>
              <button 
              ...
              <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-xs flex flex-col sm:flex-row items-center gap-5">
                <div className="relative group shrink-0">
                  <img 
                    ...
                    onClick={() => viewingAdmin.avatarUrl && setPreviewImageModal({ url: viewingAdmin.avatarUrl, title: `${viewingAdmin.name}'s Avatar` })}
                  />
                  {viewingAdmin.avatarUrl && (
                    <button
                      onClick={() => setPreviewImageModal({ url: viewingAdmin.avatarUrl!, title: `${viewingAdmin.name}'s Avatar` })}
                ...
                <div className="flex-1 text-center sm:text-left space-y-1.5">
                  <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
                    <h4 className="text-lg font-bold text-slate-900">{viewingAdmin.name}</h4>
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold border ${
                      viewingAdmin.role === 'SUPER_ADMIN'
                        ? 'bg-purple-50 text-purple-700 border-purple-200'
                        : 'bg-slate-100 text-slate-700 border-slate-200'
                    }`}>
                      {viewingAdmin.role === 'SUPER_ADMIN' && <ShieldCheck className="w-3 h-3 mr-1" />}
                  ...
                  <p className="text-xs text-slate-500 flex items-center justify-center sm:justify-start gap-1">
                    ...
                    <a href={`mailto:${viewingAdmin.email}`} className="hover:underline hover:text-slate-800">
                      {viewingAdmin.email}
                    </a>
                  </p>
              ...
              <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-xs space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                  <div>
                    ...
                    <span className="font-semibold text-slate-800 text-sm">{viewingAdmin.name}</span>
                  </div>
                  <div>
                    <span className="text-gray-400 font-semibold uppercase block text-[10px] tracking-wider mb-0.5">Email Address</span>
                    <span className="font-semibold text-slate-800 text-sm">{viewingAdmin.email}</span>
                  </div>
                  <div>
                    <span className="text-gray-400 font-semibold uppercase block text-[10px] tracking-wider mb-0.5">Role Access Level</span>
                    <span className="font-semibold text-slate-800">{viewingAdmin.role === 'SUPER_ADMIN' ? 'Super Administrator' : 'Administrator'}</span>
                  </div>
                  <div>
                    <span className="text-gray-400 font-semibold uppercase block text-[10px] tracking-wider mb-0.5">School Assigned</span>
                    <span className="font-semibold text-slate-800">{formatSchool(viewingAdmin.schoolCategory)}</span>
                  </div>
                  <div>
                    <span className="text-gray-400 font-semibold uppercase block text-[10px] tracking-wider mb-0.5">Created Date</span>
                    <span className="font-semibold text-slate-800 flex items-center gap-1">
                      <Clock className="w-3.5 h-3.5 text-slate-400" />
                      {new Date(viewingAdmin.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-400 font-semibold uppercase block text-[10px] tracking-wider mb-0.5">Gender</span>
                    <span className="font-semibold text-slate-800">{viewingAdmin.gender || 'Not specified'}</span>
                  </div>
                  {viewingAdmin.bio && (
                    <div className="sm:col-span-2">
                      <span className="text-gray-400 font-semibold uppercase block text-[10px] tracking-wider mb-0.5">Bio</span>
                      <p className="text-slate-700 bg-gray-50 p-2.5 rounded border border-gray-100 font-normal">{viewingAdmin.bio}</p>
                    </div>
                  )}
                  {viewingAdmin.socialLink && (
                    <div className="sm:col-span-2">
                      ...
                      <a href={viewingAdmin.socialLink} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline flex items-center gap-1 font-medium">
      ...
      {editingAdmin && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4 animate-in fade-in duration-150">
          <div className="bg-white rounded-sm border border-gray-200 shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleUpdateSubmit} className="p-6 overflow-y-auto custom-scrollbar space-y-4 flex-1">
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-gray-700 uppercase tracking-wider">
