...
export default function FloatingChatbot() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 1,
      text: 'Hello! I am Reyna, your AI Assistant. How can I help you?',
      sender: 'bot',
    },
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/ai/ask`,
        {
          message: userMessage,
        }
      );
      ...
      setMessages((prev) => [
        ...
        {
          id: Date.now(),
          text: 'AI assistant is not available, please try again later.',
          sender: 'bot',
        },
      ]);
    }
  ...
  return (
    <>
      ...
      >
        <div className="bg-black/90 text-white p-4 flex items-center justify-between shadow-sm z-10 border-b border-black/10">
          <div className="flex items-center gap-3.5">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-white/90 font-semibold tracking-widest uppercase opacity-90">
                AI ASSISTANT
              </span>
            </div>
          </div>
          <div className="flex items-center gap-1">
            <button
              onClick={handleReset}
              className="p-2 hover:bg-black/15 rounded-full transition-all duration-200 cursor-pointer"
              title="Reset Chat"
            >
          ...
          {messages.map((msg) => (
            ...
            >
              <div className="flex gap-2 max-w-[85%]">
                ...
                <div
                  className={`p-3 text-[13.5px] shadow-sm leading-relaxed break-words ${
                    msg.sender === 'user'
        ...
        <div className="p-3 bg-white border-t border-gray-100 flex items-center gap-2 shadow-[0_-2px_10px_rgba(0,0,0,0.02)] z-10">
          ...
          <button
            ...
            className={`w-9 h-9 rounded-full flex items-center justify-center shadow-sm flex-shrink-0 transition-all ${
              inputValue.trim()
      ...
      <div
        className={`fixed bottom-6 right-6 z-50 transition-all duration-700 ${isOpen ? 'translate-y-20 opacity-0 pointer-events-none' : 'translate-y-0 opacity-100'}`}
      >
        <div className="relative">
          {!isOpen && showTooltip && (
            <div className="absolute bottom-[110%] right-0 mb-3 w-[240px] bg-white border border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.15)] rounded-2xl p-3 pr-7 origin-bottom-right transition-all duration-300">
              ...
              <p className="text-[13px] text-gray-600 leading-snug relative z-10">
                Hey! I am{' '}
                <span className="text-[#cd2026] font-bold">Reyna</span>... Your
                AI Assistant.
              </p>
          ...
          <div className="relative">
            <button
              ...
              className={`relative flex items-center justify-center w-14 h-14 rounded-full shadow-[0_4px_15px_rgba(205,32,38,0.4)] transition-all duration-300 hover:scale-105 cursor-pointer overflow-hidden z-10 ${
                isOpen
                  ? 'bg-gray-800 shadow-[0_4px_15px_rgba(0,0,0,0.2)]'
