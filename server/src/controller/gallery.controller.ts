...
export const createAlbum = async (req: Request, res: Response) => {
  try {
    ...
    if (!name?.trim() || !category?.trim()) {
      return res
        .status(400)
        .json({ message: 'Album name and category are required' });
    }
...
export const getAllAlbums = async (req: Request, res: Response) => {
  try {
    const limitParam = Array.isArray(req.params.limit)
      ? req.params.limit[0]
      : req.params.limit;
    const offsetParam = Array.isArray(req.params.offset)
      ? req.params.offset[0]
      : req.params.offset;
  ...
  try {
    ...
    if (isBase64DataUri(image)) {
      const uploaded = await uploadBase64(
        image,
        `educlinic/gallery/albums/${albumId}`
      );
      url = uploaded.url;
...
export const addImagesToAlbum = async (req: Request, res: Response) => {
  try {
    ...
    if (images.length > 10) {
      return res
        .status(400)
        .json({ message: 'Maximum 10 images allowed per upload batch' });
    }
...
export const deleteImage = async (req: Request, res: Response) => {
  try {
    const idParam = Array.isArray(req.params.imageId)
