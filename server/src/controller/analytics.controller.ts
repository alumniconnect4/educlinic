...
export const getOverviewStats = async (req: Request, res: Response) => {
    try {
        const usersCount = await prisma.user.count({ where: { role: "USER", isVerified: true } });
        const alumniCount = await prisma.user.count({ where: { role: "ALUMNI", isVerified: true } });
        const adminCount = await prisma.user.count({ where: { role: "ADMIN", isVerified: true } });
...
export const getUserRegistrationAnalytics = async (req: Request, res: Response) => {
    try {
        ...
        const roleStats = await Promise.all(
            roles.map(async (role) => {
                const total = await prisma.user.count({ where: { role, isVerified: true } });
                return { role, total };
            })
        );
