...
import { invalidateUsersCache, invalidatePostsCache } from '../config/cache.js';
...
async function migrateImages() {
  ...
  for (const user of users) {
    ...
    if (user.avatarUrl) {
      if (
        ...
        resetAvatars++;
        console.log(
          `[User ${user.id} - ${user.email}] Replaced SVG avatar with DEFAULT_AVATAR_URL`
        );
      } else if (isBase64Image(user.avatarUrl)) {
        try {
          console.log(
            `[User ${user.id} - ${user.email}] Uploading base64 avatar to Cloudinary...`
          );
          ...
          migratedAvatars++;
          console.log(
            `[User ${user.id} - ${user.email}] Avatar uploaded successfully: ${uploadRes.secure_url}`
          );
        } catch (err) {
          console.error(
            `[User ${user.id} - ${user.email}] Failed to upload avatar to Cloudinary, resetting to default:`,
            err
          );
          updateData.avatarUrl = DEFAULT_AVATAR_URL;
          resetAvatars++;
    ...
    if (user.idCardUrl && isBase64Image(user.idCardUrl)) {
      try {
        console.log(
          `[User ${user.id} - ${user.email}] Uploading base64 ID card to Cloudinary...`
        );
        ...
        migratedIdCards++;
        console.log(
          `[User ${user.id} - ${user.email}] ID Card uploaded: ${uploadRes.secure_url}`
        );
      } catch (err) {
        console.error(
          `[User ${user.id} - ${user.email}] Failed to upload ID Card, resetting to null:`,
          err
        );
    ...
    if (user.degreeUrl && isBase64Image(user.degreeUrl)) {
      try {
        console.log(
          `[User ${user.id} - ${user.email}] Uploading base64 Degree Certificate to Cloudinary...`
        );
        ...
        migratedDegrees++;
        console.log(
          `[User ${user.id} - ${user.email}] Degree Certificate uploaded: ${uploadRes.secure_url}`
        );
      } catch (err) {
        console.error(
          `[User ${user.id} - ${user.email}] Failed to upload Degree Certificate, resetting to null:`,
          err
        );
  ...
  for (const post of posts) {
    if (post.imageUrl && isBase64Image(post.imageUrl)) {
      try {
        console.log(
          `[Post ${post.id}] Uploading base64 image to Cloudinary...`
        );
        ...
        migratedPosts++;
        console.log(
          `[Post ${post.id}] Post image uploaded: ${uploadRes.secure_url}`
        );
      } catch (err) {
        console.error(
          `[Post ${post.id}] Failed to upload image, removing base64:`,
          err
        );
        await prisma.post.update({
          where: { id: post.id },
  ...
  for (const event of events) {
    if (event.imageUrl && isBase64Image(event.imageUrl)) {
      try {
        console.log(
          `[Event ${event.id}] Uploading base64 image to Cloudinary...`
        );
        ...
        migratedEvents++;
        console.log(
          `[Event ${event.id}] Event image uploaded: ${uploadRes.secure_url}`
        );
      } catch (err) {
        console.error(
          `[Event ${event.id}] Failed to upload image, resetting to default:`,
          err
        );
      }
    }
